...
| Область | Кратко |
|--------|--------|
| **Лента** | RSS, резервные каналы загрузки, предзагрузка в несколько слотов |
| **Фильтры** | Минимальное разрешение, ориентация, скрытие NSFW, опция «не повторять» |
| **Интервал** | По умолчанию **каждые 30 минут**; в настройках задаётся в **минутах** (устойчивый таймер после сна ПК) |
| **Кэш** | Папка с `wp_*.jpg` и запасом — по умолчанию каталог данных приложения; можно задать свой путь (`~` поддерживается) |
